...
// La siguiente funcion a sido deshabilitada debido a que el elemento calefactable es poco potente, y se a decidido mantener el ventilador encendido o parado, unicamente, al maximo o al minimo.
// Este codigo controlaba la velocidad del ventilador, reduciendo o aumentando la velocidad en funcion de la humedad.
 /*
int i = 65; // Puntos de control de humedad fisica posible.
  pulso = (HUM * T) + i;    // Ajusta el ancho del pulso segun humedad añadiendo "i" puntos teniendo en cuenta una humedad improbable del 60%
   if (pulso != ultimo_pulso) {  // Si cambia la humedad
       ultimo_pulso = pulso; // Imprime cambios
       
   }      while (pulso > 255); {        // Si le formula sobrepasa la velocidad de pulso del ventilador, reduce progresivamente 1 punto hasta su maximo.
        pulso--;   //Corrige un posible ambiente extremadamente humedo que supere el 60%, algo poco habitual en una casa. Esto sirve como corrector de un posible bug.
       }

   } else {
       digitalWrite(fan, HIGH);  
       delayMicroseconds(pulso);   // Mantiene el valor alto el tiempo de pulso
       digitalWrite(fan, LOW);   
       delayMicroseconds(pulso);   // Valor bajo durante el tiempo de pulso
     }

    if (HUM <= Hcontrol) { // Si se llega a la Humedad de control, apaga.
      digitalWrite(coldled, LOW);
      digitalWrite(warmled, HIGH);
      digitalWrite(relay, LOW);
   } 
